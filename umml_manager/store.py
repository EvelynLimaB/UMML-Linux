...
class ManagerStore:
    ...
    def list_mods(self) -> list[ModRecord]:
        data = _read_json_object(self.paths.registry, {"version": REGISTRY_VERSION, "mods": []}, strict=True)
        raw = data.get("mods", [])
        if not isinstance(raw, list):
            raise StoreError(f"Manager mod registry has an invalid mods list: {self.paths.registry}")
        try:
            records = [ModRecord.from_dict(item) for item in raw]
        except (TypeError, ValueError, KeyError) as exc:
            raise StoreError(f"Manager mod registry contains an invalid record: {exc}") from exc
        seen: set[str] = set()
        for record in records:
            if record.id in seen:
                raise StoreError(f"Manager mod registry contains duplicate ID: {record.id}")
            seen.add(record.id)
        return records
    ...
    def save_mod(self, record: ModRecord) -> None:
        ...
        try:
            with FileLock(self.paths.locks / "registry.lock", purpose="updating the mod library"):
                ...
                _write_document(
                    self.paths.registry,
                    {
                        ...
                        "mods": [mods[key].to_dict() for key in sorted(mods)],
                    },
                )
    ...
    def list_profiles(self) -> list[Profile]:
        ...
        if not isinstance(raw, list):
            raise StoreError(f"Manager profile registry has an invalid profiles list: {self.paths.profiles}")
        try:
            profiles = [Profile.from_dict(item) for item in raw]
        except (TypeError, ValueError, KeyError) as exc:
            raise StoreError(f"Manager profile registry contains an invalid profile: {exc}") from exc
        seen: set[str] = set()
        for profile in profiles:
            if not profile.name.strip():
                raise StoreError("Manager profile registry contains an empty profile name")
            if profile.name in seen:
                raise StoreError(f"Manager profile registry contains duplicate name: {profile.name}")
    ...
    def save_profile(self, profile: Profile) -> None:
        ...
        try:
            with FileLock(self.paths.locks / "profiles.lock", purpose="updating profiles"):
    ...
    def load_settings(self) -> dict[str, Any]:
        return dict(_read_json_object(self.paths.settings, {}, strict=False))

    def save_settings(self, values: dict[str, Any]) -> None:
        try:
            with FileLock(self.paths.locks / "settings.lock", purpose="updating settings"):
    ...
    def source_destination(self, mod_id: str, version: str) -> Path:
        return self.paths.sources / sanitize_id(mod_id) / storage_component(version)

    def prepared_destination(self, record: ModRecord) -> Path:
        return self.paths.prepared / sanitize_id(record.id) / storage_component(record.version)
    ...
    def import_folder(
        ...
        *,
        ...
        base_id = sanitize_id(mod_id or metadata.get("id") or metadata.get("title") or selected.name)
        ...
        try:
            ...
            if destination.exists():
                ...
                _copy_tree_atomic(selected, destination)
        except SafetyError as exc:
            raise StoreError(str(exc)) from exc
        ...
        record = ModRecord(
            ...
            incompatibilities=_string_values(metadata.get("incompatibilities", [])),
        )
    ...
    def import_archive(
        ...
        *,
        ...
        if not stat.S_ISREG(mode):
            raise StoreError(f"Archive is not a regular file: {archive_path}")
        ...
        archive_source = replace(
            ...
            size_bytes=archive_source.size_bytes or archive_path.stat().st_size,
            fetched_at=archive_source.fetched_at or _now(),
        )
        temporary = Path(tempfile.mkdtemp(prefix="umml-import-", dir=self.paths.root))
        try:
    ...
    def create_workspace(self, mod_id: str) -> Path:
        ...
        destination = self.paths.workspaces / sanitize_id(mod_id) / stamp
        ...
        atomic_write_json(destination / ".umml-workspace.json", marker)
        return destination
...
def sanitize_id(value: str) -> str:
    cleaned = re.sub(r"[^a-z0-9._-]+", "-", str(value).casefold()).strip("-.")
    if not cleaned:
    ...
    for name in ("umml-mod.json", "setting.json"):
        ...
        if path.is_file():
            ...
            if not isinstance(data, dict):
                raise StoreError(f"Invalid {name}: top-level value must be an object")
            return data
...
def find_mod_root(extracted: Path) -> Path:
    if is_mod_root(extracted):
        return extracted
    candidates = scan_mod_candidates([extracted], max_depth=8, include_archives=False)
    if not candidates:
        raise StoreError(f"No recognizable UMML/Hachimi mod folder found under {extracted}")
    minimum_depth = min(len(item.path.relative_to(extracted).parts) for item in candidates)
    nearest = [item for item in candidates if len(item.path.relative_to(extracted).parts) == minimum_depth]
    if len(nearest) > 1:
        names = ", ".join(str(item.path.relative_to(extracted)) for item in nearest[:5])
        raise StoreError(
            "Multiple mod folders were found. Import them separately instead of guessing: " + names
        )
...
def _extract_zip(archive: Path, destination: Path) -> None:
    with zipfile.ZipFile(archive) as package:
        ...
        _check_archive_limits(archive, len(members), sum(max(0, member.file_size) for member in members))
        seen: set[str] = set()
        actual = 0
        for member in members:
            relative = _safe_member(member.filename)
            if relative in seen and not member.is_dir():
                raise StoreError(f"Archive contains duplicate file path: {relative}")
            seen.add(relative)
            if member.flag_bits & 0x1:
                raise StoreError(f"Encrypted ZIP entry is unsupported: {member.filename}")
            mode_type = stat.S_IFMT(member.external_attr >> 16)
            if mode_type not in {0, stat.S_IFREG, stat.S_IFDIR}:
                raise StoreError(f"Archive special file rejected: {member.filename}")
            target = path_under(destination, relative)
            if member.is_dir() or member.filename.endswith("/"):
                target.mkdir(parents=True, exist_ok=True)
                continue
            with package.open(member, "r") as source:
                actual = _extract_stream(source, target, actual, archive)
    ...
    with tarfile.open(archive) as package:
        ...
        for member in members:
            ...
            if not (member.isfile() or member.isdir()):
                raise StoreError(f"Archive link, device, or special file rejected: {member.name}")
            target = path_under(destination, relative)
            if member.isdir():
                target.mkdir(parents=True, exist_ok=True)
                continue
            ...
            if source is None:
                raise StoreError(f"Could not read archive member: {member.name}")
            with source:
                actual = _extract_stream(source, target, actual, archive)


def _extract_stream(source: BinaryIO, target: Path, total: int, archive: Path) -> int:
        ...
        with os.fdopen(fd, "wb") as output:
            while True:
                ...
                if total > MAX_ARCHIVE_UNCOMPRESSED_BYTES:
                    raise StoreError(
                        f"Archive expanded beyond the {MAX_ARCHIVE_UNCOMPRESSED_BYTES // (1024 ** 3)} GiB safety limit: {archive.name}"
                    )
                output.write(chunk)
            output.flush()
...
def _safe_member(name: str) -> str:
    ...
    try:
        ...
        raise StoreError(f"Unsafe archive path rejected: {name!r}") from exc


def _check_archive_limits(archive: Path, count: int, expanded_bytes: int) -> None:
    if count > MAX_ARCHIVE_ENTRIES:
        raise StoreError(
            f"Archive contains {count:,} entries; the safety limit is {MAX_ARCHIVE_ENTRIES:,}: {archive.name}"
        )
    if expanded_bytes > MAX_ARCHIVE_UNCOMPRESSED_BYTES:
        ...
        raise StoreError(
            f"Archive expands to {gib:.2f} GiB; the safety limit is {limit:.0f} GiB: {archive.name}"
        )
...
def _copy_tree_atomic(source: Path, destination: Path) -> None:
    ...
    try:
        ...
        shutil.copytree(source, content, symlinks=False)
...
def _read_json_object(path: Path, default: dict[str, Any], *, strict: bool) -> dict[str, Any]:
    if not path.is_file():
        return dict(default)
    try:
        data = json.loads(path.read_text(encoding="utf-8"))
        ...
        if strict:
            raise StoreError(f"Manager state file is unreadable: {path}. It was not replaced or reset.") from exc
        return dict(default)
    if not isinstance(data, dict):
        if strict:
            raise StoreError(f"Manager state file must contain an object: {path}")
        return dict(default)
    return data
...
def _write_document(path: Path, data: dict[str, Any]) -> None:
    if path.is_file():
        try:
            ...
            raise StoreError(f"Could not preserve state backup for {path}: {exc}") from exc
...
def _package_type(root: Path) -> str:
    ...
    if hachimi.is_dir() and any(item.is_file() for item in hachimi.rglob("*")):
...
def _validate_record_identity(record: ModRecord) -> None:
    if sanitize_id(record.id) != record.id:
        raise StoreError(f"Mod record has an unsafe ID: {record.id!r}")
    if not record.name.strip():
        raise StoreError(f"Mod record has an empty name: {record.id}")
