...
class SystemActions:
    def launch_legacy_tool(self, tool_id: str):
        try:
            LegacyToolLauncher().launch(tool_id, dat_path=self.dat_path.get(), game_dir=self.game_dir.get(), meta_path=self.meta_path.get(), region=self.region.get())
            self.status.set("Opened UMML Studio compatibility host")
        except Exception as exc:
            messagebox.showerror("Could not open legacy tool", str(exc), parent=self.root)
    ...
    def choose_meta(self):
        path = filedialog.askopenfilename(parent=self.root, filetypes=(("Database", "*.db meta*"), ("All", "*")))
        if path:
    ...
    def save_settings(self, silent: bool = False):
        roots = [item.strip() for item in self.scan_roots.get().split(";") if item.strip()]
        self.store.save_settings({"profile": self.profile_name.get(), "dat_path": self.dat_path.get(), "meta_path": self.meta_path.get(), "game_dir": self.game_dir.get(), "region": self.region.get(), "gamebanana_region": self.gb_region.get(), "scan_roots": roots})
        if not silent:
    ...
    def run_diagnostics(self):
        ...
        box = tk.Text(window, wrap="none", background=SURFACE, foreground=TEXT, insertbackground=TEXT, font=("TkFixedFont", 10))
        box.pack(fill="both", expand=True, padx=12, pady=12)
        box.insert("1.0", report)
        box.configure(state="disabled")
        self.status.set("Diagnostics READY" if ready else "Diagnostics found incomplete setup")
    ...
    def _run_task(self, label: str, operation: Callable[[], Any], completed: Callable[[Any], None]):
        if self._busy:
            messagebox.showinfo("UMML is busy", "Another operation is still running.", parent=self.root)
            return
        self._busy = True
        ...
        def worker():
            try:
                ...
                self.root.after(0, lambda: self._task_completed(result, completed))
        threading.Thread(target=worker, daemon=True).start()

    def _task_completed(self, result: Any, completed: Callable[[Any], None]):
        self._busy = False
        self.progress.stop()
        try:
            completed(result)
    ...
    def _task_failed(self, exc: Exception):
        self._busy = False
        self.progress.stop()
